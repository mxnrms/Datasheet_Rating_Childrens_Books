---
title: "Datasheet for Children's Book Ratings"
author: 
  - Aliyah Maxine Ramos
thanks: "Code and data are available at: https://github.com/mxnrms/Tutorial_10.git"
date: "`r Sys.time()`"
date-format: "D MMMM YYYY"
abstract: "This datasheet describes Alex Cookson's data on Children's Book Ratings. This dataset is comprised of textual documents with the ratings of children's books and it is also made publicly available on GitHub."
format:
  pdf:
...
The dataset of interest is Alex Cookson's data on the ratings of children's books [@alexcookson_childrensbookratings]. The dataset is publicly available on GitHub, alongside other datasets that Cookson has used for personal projects, which are usually associated with blog posts on his website[^1]. The Children's Book Ratings dataset is comprised of a README file and 3 textual documents, which display the variables and data relating to the ratings for approximately 9000 children's books. For the datasheet of this dataset, I follow the recommendations of Gebru et al. [@gebru2021datasheets].

[^1]: https://www.alexcookson.com/

# Full Datasheet for Children's Book Ratings

## Motivation

-   **For what purpose was the dataset created? Was there a specific task in mind? Was there a specific gap that needed to be filled? Please provide a description.**
    -   The dataset was created to address the issue of inconsistency on rating sites. This problem arises when different rating platforms, such as Rotten Tomatoes or Amazon, have identical ratings but have varying implications, causing an uncertainty for which rating to trust. It was also created to estimate more reasonable ratings, specifically for children's books, by using empirical Bayes estimation.
-   **Who created the dataset (for example, which team, research group) and on behalf of which entity (for example, company, institution, organization)?**
    -   Alex Cookson created this dataset as a personal project, while working at the Royal Canadian Mint.
...
-   **What do the instances that comprise the dataset represent (for example, documents, photos, people, countries)? Are there multiple types of instances (for example, movies, users, and ratings; people and interactions between them; nodes and edges)? Please provide a description.**
    -   The dataset consists of textual documents. All textual documents are formatted in the same way. Each row in the dataset represents a children's book that includes various information about that book, such as its title and author.
...
-   **Does the dataset contain all possible instances or is it a sample (not necessarily random) of instances from a larger set? If the dataset is a sample, then what is the larger set? Is the sample representative of the larger set (for example, geographic coverage)? If so, please describe how this representativeness was validated/verified. If it is not representative of the larger set, please describe why not (for example, to cover a more diverse range of instances, because instances were withheld or unavailable).**
    -   The dataset specifies that it contains approximately 9000 children's books that have been rated from one to five stars. It is not known whether the dataset contains all the possible instances or is a sample of children's books that have received a one to five star rating.
-   **What data does each instance consist of? "Raw" data (for example, unprocessed text or images) or features? In either case, please provide a description.**
    -   Each instance consists of a ten-digit International Standard Book Number (ISBN), the book title, the author, the number of individuals that gave the book a rating, and the ratings, which are measured out of 5 stars. The "raw" data consists of the features mentioned previously, as well as unadjusted ratings and some metadata, such as cover type and publisher.
...
-   **Are relationships between individual instances made explicit (for example, users' movie ratings, social network links)? If so, please describe how these relationships are made explicit.**
    -   Yes, the relationships between individual instances are made explicit through the unique ten-digit ISBN.
...
-   **Is the dataset self-contained, or does it link to or otherwise rely on external resources (for example, websites, tweets, other datasets)? If it links to or relies on external resources, a) are there guarantees that they will exist, and remain constant, over time; b) are there official archival versions of the complete dataset (that is, including the external resources as they existed at the time the dataset was created); c) are there any restrictions (for example, licenses, fees) associated with any of the external resources that might apply to a dataset consumer? Please provide descriptions of all external resources and any restrictions associated with them, as well as links or other access points, as appropriate.**
    -   The dataset is linked to a blogpost on Alex Cookson's website, which is titled, "Rating children's books with empirical Bayes estimation (1 of 2)" [@alexcookson_blogpost]. There are no restrictions associated with the external source.
...
-   **What mechanisms or procedures were used to collect the data (for example, hardware apparatuses or sensors, manual human curation, software programs, software APIs)? How were these mechanisms or procedures validated?**
    -   Cookson notes that when using data for his personal projects, he engages in web scraping and cleaning, or a combination of these techniques using R.
...
-   **Has an analysis of the potential impact of the dataset and its use on data subjects (for example, a data protection impact analysis) been conducted? If so, please provide a description of this analysis, including the outcomes, as well as a link or other access point to any supporting documentation.**
    -   Some forms of statistical bias when giving a book a rating were noted, such as self-selection bias [@alexcookson_blogpost].
-   **Any other comments?**
    -   No.
...
-   **Has the dataset been used for any tasks already? If so, please provide a description.**
    -   The dataset has been used for a blogpost on Alex Cookson's website [@alexcookson_blogpost].
-   **Is there a repository that links to any or all papers or systems that use the dataset? If so, please provide a link or other access point.**
    -   No.
-   **What (other) tasks could the dataset be used for?**
    -   The dataset could be used for any type of use.
...
-   **How can the owner/curator/manager of the dataset be contacted (for example, email address)?**
    -   Alex can be contacted directly through his Twitter[^2] or through his website[^3].
...
[^2]: https://twitter.com/alexcookson

[^3]: https://www.alexcookson.com/
